...
var colorsNeedToFindItsCount = [];
var bagWithoutCommasArr = [];
var count = 0;
var colorsObj = {};
var uniqueArr;
var arrOfArraysNew = [];
...
bagsRules.forEach(bag => {
    var everyBagArr = bag.split(',');
    var bagArr = bag.split(',')[0].split(' ');
    var bagWithoutCommas = bag.split(',');
    var arr = bagWithoutCommas[0].split('contain');

    bagWithoutCommas.push(arr[1]);
    bagWithoutCommas[0] = arr[0];
    bagWithoutCommasArr.push(bagWithoutCommas);

    everyBagArr.forEach(() => {
        if (bag.includes('shiny gold bag') && !bag.includes('shiny gold bags contain')) {
            var colorsWhichIncludeGoldBag = bagArr[0] + ' ' + bagArr[1] + ' ' + bagArr[2].slice(0, -1);
            if (!colorsObj[colorsWhichIncludeGoldBag]) {
                colorsObj[colorsWhichIncludeGoldBag] = 1;
                count += 1;
            }
            colorsNeedToFindItsCount.push(colorsWhichIncludeGoldBag);
            bagArr.forEach(bagEvery => {
                var eachBag = bagEvery.split(' ');
                eachBag.forEach((bag, index) => {
                    if (bag == '') {
                        eachBag.splice(index, 1);
                    }
                });
            });
        }
...
function unique(arr) {
    ...
    arrOfArraysNew.push(uniqueArr);
    bagWithoutCommasArr.forEach(arrBags => {
        for (var i = 1; i < arrBags.length; i++) {
            uniqueArr.forEach(color => {
                if (arrBags[i].includes(color)) {
                    var colorName = arrBags[0].slice(0, -2);
                    newColorsToFind.push(colorName);
                    if (!colorsObj[colorName]) {
                        colorsObj[colorName] = 1;
                        count += 1;
                    }
                }
            });
    ...
    uniqueArr = newColorsToFind;
    function uniqueNew(arr) {
        var uniqueArrNew = Array.from(new Set(arr));
        if (newColorsToFind.length != 0) {
            unique(uniqueArrNew);
        }
    }
    uniqueNew(uniqueArr);
...
unique(colorsNeedToFindItsCount);
