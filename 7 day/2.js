...
var namesObj = {};
var colorNamesToFind = ['shiny gold bags contain'];

function getNewColorsNames(colors) {
    // console.log('colors ', colors);

    var newNamesArr = [];
    bagsRules.forEach(bag => {
        var onlyColorsTypes;
        // console.log('bag ', bag);

        for (var i = 0; i < colors.length; i++) {
            // colors.forEach(nameColor => {
            // console.log('bag ', bag);
            // console.log(' ');
            if (bag.includes(colors[i])) {

                var bagSplitedByContain = bag.split('contain ');
                var bagSplited = bag.split(' ');
                // console.log('bagSplited ', bagSplited);
                var parentBagName = bagSplited[0] + ' ' + bagSplited[1];

                onlyColorsTypes = bagSplitedByContain[1];
                // console.log('onlyColorsTypes ', onlyColorsTypes);

                if (onlyColorsTypes != 'no other bags.') {
                    var colorsSplitedByComma = onlyColorsTypes.split(', ');
                    // console.log('colorsSplitedByComma ', colorsSplitedByComma);

                    colorsSplitedByComma.forEach(color => {
                        var splitedColorName = color.split(' ');
                        // console.log('color ', color);
                        var name = splitedColorName[1] + ' ' + splitedColorName[2];
                        var count = splitedColorName[0];
                        if (!namesObj[parentBagName]) {
                            console.log('parentBagName ', parentBagName);
                            console.log('name ', name);
                            console.log('count ', count);
                            console.log(' ');
                            namesObj[parentBagName] = [[name, count]];
                            newNamesArr.push(name);
                        } else {
                            namesObj[parentBagName].push([name, count]);
                        }
                    });
                    // colorNamesToFind = newNamesArr;
                    // console.log('colorNamesToFind ', colorNamesToFind);
                }
            }
            // });
        }
    ...
    if (newNamesArr.length > 0) {
        // getNewColorsNames(newNamesArr);
        getNewColorsNames(newNamesArr);
    }
}

getNewColorsNames(colorNamesToFind);

// function findColors(bag, colors) {

// }

// getNewColorsNames(colorNamesToFind);
// findColors(colorNamesToFind);
console.log('length ', Object.keys(namesObj).length);

for (var key in namesObj) {
    console.log('namesObj[key] ', namesObj[key]);
    uniqueNew(namesObj[key], key);
}

function uniqueNew(arr, key) {
    var uniqueArrNew = Array.from(new Set(arr));
    namesObj[key] = uniqueArrNew;
}
